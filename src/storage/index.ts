...
export const storage = {
  ...
  async addHistoryEntry(entry: HistoryEntry): Promise<void> {
    const history = await get<HistoryEntry[]>(KEYS.HISTORY, []);
  ...
  // Templates
  getTemplates: () => get<Template[]>(KEYS.TEMPLATES, []),
  async saveTemplate(template: Template): Promise<void> {
    const templates = await get<Template[]>(KEYS.TEMPLATES, []);
  ...
  async importHistory(json: string): Promise<void> {
    ...
    const current = await get<HistoryEntry[]>(KEYS.HISTORY, []);
